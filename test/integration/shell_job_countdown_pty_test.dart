...
void main() {
  test('ten background bash jobs start, count down on camera, drain to '
    ...
    final project = Directory('/tmp/fa_573_proj')..createSync(recursive: true);
    ...
    // Anchored polling (#533/#550/#557 deflake precedent), NOT the
    // settle window: settles land ~1s apart by design (sleeps 3..12),
    // which sits right on the harness's 2×500ms output-settle
    // early-return threshold — under CI-runner load a repaint gap past
    // that threshold returned after only the first few notices
    // (observed: {1,2,3} of 10, run 36204293573 shard 2/3). Poll the
    // raw buffer for the distinct settled set; the 60s bound and the
    // full-set contract stand.
    //
    // Match on the NUMBER PREFIX, never the full id: a repaint can
    // interleave cursor-move escapes mid-id (observed under CI load:
    // `sh-10-…xqo<esc>[11;29H5<esc>[11;31H exited(0)`), and a
    // fragmented re-emit would otherwise inflate the distinct-id set
    // with a corrupt entry. The number is the first token written, so
    // it survives any such fragmentation.
    final drainDeadline = DateTime.now().add(const Duration(seconds: 60));
    Set<String> settledNumbers = {};
    while (!settledNumbers.containsAll({for (var i = 1; i <= 10; i++) '$i'})) {
      if (!DateTime.now().isBefore(drainDeadline)) break;
      await Future<void>.delayed(const Duration(milliseconds: 50));
      settledNumbers = _settleNotice
          .allMatches(harness.rawOutput)
