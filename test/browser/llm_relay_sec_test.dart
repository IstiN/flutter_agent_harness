...
/// An SSE-answering client that records every outbound request.
...
LlmRelayRequest _req(String baseUrl, {String? provider}) =>
    LlmRelayRequest(
        baseUrl: baseUrl,
        model: 'glm-4.6',
        messages: [
...
void main() {
  group('resolveLlmRelayTarget — the SEC-01 server-side resolution', () {
    ...
    test('a keyless record resolves with a null key (frame glue rejects)',
        () {
      final target = _resolver(_req('https://lan.example/v1',
          provider: 'lanbox'));
      expect(target!.rejected, isFalse);
  ...
  group('SEC-01 AC1 — a keyed request never goes to a client-chosen address',
      () {
    ...
    test('unknown provider → zero outbound requests + named error',
      ...
      final frames = await _handle(
          _req(_recordZai, provider: 'ghost'), client);
      expect(client.requests, isEmpty);
    ...
    test('named provider without a stored key → zero outbound + no-key '
      ...
      final client = _CountingClient();
      final frames = await _handle(
          _req('https://lan.example/v1', provider: 'lanbox'), client);
      expect(client.requests, isEmpty);
  ...
  group('SEC-01 AC2 — a named provider request goes to the record', () {
    test('the record baseUrl is used verbatim with the record key attached',
        () async {
      final client = _CountingClient();
  ...
  group('SEC-01 AC3 — cross-origin redirect strips the relayed auth', () {
    test('a 302 to another host fails; no request re-sent, no auth leaked',
        () async {
      final client = _CountingClient();
  ...
  group('REG (SEC-01 AC4) — no Bearer may leave to a non-record address',
    ...
    /// client could send, honest or malicious.
    final fixtures = <String, LlmRelayRequest>{
      'named+record': _req(_recordZai, provider: 'zai'),
      'named+attacker': _req(_attacker, provider: 'zai'),
      'second-record': _req(_recordOr, provider: 'orai'),
      'unknown-provider': _req(_recordZai, provider: 'ghost'),
      'keyless-record': _req('https://lan.example/v1', provider: 'lanbox'),
      'anonymous+record': _req(_recordZai),
      'anonymous+attacker': _req(_attacker),
    };
    ...
    test('property over all fixtures: a Bearer only ever rides to a stored '
      ...
      final client = _CountingClient();
      final bearerTargets = <String>[];
      for (final entry in fixtures.entries) {
        final frames = await _handle(entry.value, client);
        final error = frames
            .where((f) => f.fields['error'] != null)
            .map((f) => f.fields['error'])
            .join();
        // A keyed fixture either rejects (error, zero requests for THIS
        // fixture) or sends exactly the record's call — checked below as
        // the global property over every request the suite produced.
        assert(error.isEmpty || error.isNotEmpty);
      }
      for (final request in client.requests) {
        final auth = request.headers['authorization'];
        if (auth != null && auth.startsWith('Bearer ')) {
          bearerTargets.add(request.url.toString());
        }
      }
      for (final target in bearerTargets) {
        expect(recordBaseUrls.any(target.startsWith), isTrue,
            reason: 'a Bearer left for "$target" — not a stored record '
                'baseUrl');
      }
    ...
    test('every fixture yields either a done frame or an error frame — '
        'never silence', () async {
      for (final entry in fixtures.entries) {
        final frames = await _handle(entry.value, _CountingClient());
        expect(frames, isNotEmpty, reason: '${entry.key}: silent relay');
        expect(
          frames.last.fields['done'] == true ||
              frames.last.fields['error'] != null,
          isTrue,
          reason: '${entry.key}: terminal frame missing',
        );
      }
    });
  });
