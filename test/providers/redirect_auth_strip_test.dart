...
void main() {
  group(
    'sendProviderRequest — cross-origin redirect auth-stripping (SEC-01)',
    () {
      test(
        'a 302 to another host FAILS: exactly one request, never re-sent',
        () async {
          ...
          expect(
            ...
            hasLength(1),
            reason:
                'the request must never be re-issued at the cross-origin '
                'target',
          );
      ...
      test(
        'the failure formats as the familiar redirect/SSO diagnosis',
        () async {
          final client = _RecordingClient(
            (call, request) async => _empty(302, {
              'location': 'https://login.example.org/sso?rd=%2Fv1',
            }),
          );
          try {
      ...
      test(
        'a same-origin 302 is followed verbatim: method, body, auth',
        () async {
          final client = _RecordingClient(
            (call, request) async => call == 1
      ...
      test(
        'the same-origin follow loop is capped (5 hops, then an error)',
        () async {
          final client = _RecordingClient(
            (call, request) async => _empty(302, {
              'location': 'https://api.example.com/v1/hop$call',
            }),
          );
          await expectLater(
            sendProviderRequest(
      ...
      test(
        'the 200-with-HTML SSO guard still applies after a followed hop',
        () async {
          final client = _RecordingClient(
            (call, request) async => call == 1
