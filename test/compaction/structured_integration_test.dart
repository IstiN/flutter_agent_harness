...
void main() {
  ...
  group('#729 — fit-the-window payloads', () {
    test('AC1: a region over the summarizer window is folded chunk-wise; '
        'every checkpoint prompt fits the summarizer window', () async {
      final session = await repo.create(
        JsonlSessionCreateOptions(cwd: '/work'),
      );
      ...
      for (var i = 0; i < 6; i++) {
        await session.appendMessage(UserMessage.text('ask $i ${'a' * 3000}'));
        await session.appendMessage(
          _assistant(
            'step $i',
            calls: [ToolCall(id: 'c$i', name: 'read', arguments: {})],
    ...
    test(
      'a mid-fold summarizer failure stops the fold (failure-safe)',
      () async {
        final session = await repo.create(
          JsonlSessionCreateOptions(cwd: '/work'),
        );
