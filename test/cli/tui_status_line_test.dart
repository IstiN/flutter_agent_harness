...
void main() {
  group('segment registry', () {
    ...
    test('hidden-by-design ids stay valid but render nothing', () {
      ...
      for (final id in ['usage', 'collab', 'vim', 'cache_hit']) {
        expect(kStatusLineSegments[id]!(s, _defaultSpec()), isNull,
    ...
    test('model segment carries the thinking-level suffix', () {
      ...
      const leveled = StatusLineSnapshot(
          cwd: '/', modelName: 'm1', thinkingLevel: 'high');
      expect(
        kStatusLineSegments['model']!(leveled, _defaultSpec())!.text,
        'm1 · high',
      );
      final noSuffixSpec = resolveStatusLineSpec(const StatusLineConfig(
        segmentOptions: StatusLineSegmentOptions(
            modelShowThinkingLevel: false),
      ));
      expect(
        kStatusLineSegments['model']!(leveled, noSuffixSpec)!.text,
        'm1',
      );
    });
  });
  ...
  group('presets', () {
    ...
    test('shapes pin the omp table', () {
      ...
      expect(
        kStatusLinePresets['default']!.right,
        ['session_name'],
      );
      expect(
      ...
      expect(kStatusLinePresets['ascii']!.separator,
    ...
    test('resolve defaults, explicit preset and custom implication', () {
      ...
      final minimal =
          resolveStatusLineSpec(const StatusLineConfig(preset: 'minimal'));
      expect(minimal.left, kStatusLinePresets['minimal']!.left);
      final custom = resolveStatusLineSpec(const StatusLineConfig(
        left: ['model'],
        right: ['cost'],
      ));
      // Explicit groups without a preset imply the custom base.
      expect(custom.separator,
          kStatusLinePresets['custom']!.separator);
  ...
  group('separators', () {
    test('all 7 styles resolve glyphs', () {
      expect(
          getSeparator(StatusLineSeparatorStyle.powerline).left, '▶');
      expect(
          getSeparator(StatusLineSeparatorStyle.powerline).capAfterLeft, '◀');
      expect(
          getSeparator(StatusLineSeparatorStyle.powerlineThin).left, '>');
    ...
    test('nerd table swaps the powerline and block glyphs', () {
      final nerd =
          getSeparator(StatusLineSeparatorStyle.powerline, nerd: true);
      expect(nerd.left.length, 1);
      expect(nerd.left.codeUnitAt(0), greaterThan(0xE000),
          reason: 'nerd glyphs live in the private-use area');
      expect(getSeparator(StatusLineSeparatorStyle.slash, nerd: true).left,
          '/');
      expect(getSeparator(StatusLineSeparatorStyle.block, nerd: true).left,
          '█');
    });

    test('parse and strict errors', () {
      expect(parseStatusLineSeparator('slash'),
  ...
  group('truncation ladder (widths 40..200)', () {
    ...
    test('step 4: left drops left-to-right last', () {
      ...
      final line =
          renderStatusLine(ladderSnapshot, costOnly, 24).single.trim();
      expect(line, '\$1.25');
    ...
    test('empty groups render nothing', () {
      const empty = StatusLineSnapshot(cwd: '/');
      final blank = resolveStatusLineSpec(const StatusLineConfig(
        preset: 'custom',
        left: ['usage'],
        right: ['vim'],
      ));
  ...
  group('context gauge', () {
    ...
    test('minimum width covers both labels plus the bridge', () {
      expect(statusLineGaugeMinWidth(42, 10000),
    ...
    test('fill lands between the groups at the right level', () {
      ...
      expect(gaugeRoles, {StatusLineRoleKey.gaugeUsed, StatusLineRoleKey.gaugeUnused});

      final warnSpans = renderStatusLineSpans(
        snapshot,
        spec,
        160,
      );
      // Level role appears on the percent label.
      final labelRole = warnSpans
          .firstWhere((s) => s.$1.trim() == '42%')
          .$2;
      ...
      expect(
        renderStatusLineSpans(at(60), spec, 160)
            .firstWhere((s) => s.$1.trim() == '60%')
            .$2,
        StatusLineRoleKey.warn,
      );
      expect(
        renderStatusLineSpans(at(95), spec, 160)
            .firstWhere((s) => s.$1.trim() == '95%')
            .$2,
  ...
  group('config parse', () {
    test('valid full section', () {
      final tui = parseTuiSection(_tui('''
tui:
  theme: catppuccin
  classic: true
  statusLine:
...
'''));
    ...
    test(' CliConfig parses and round-trips the tui section', () {
      final config = parseTuiSection(_tui('''
tui:
  statusLine:
    preset: ascii
'''));
    ...
    test('strict errors', () {
      expect(() => parseTuiSection(_tui('tui: nope')),
          throwsA(isA<ConfigException>()));
      expect(() => parseTuiSection(_tui('tui:\n  bogus: 1')),
          throwsA(isA<ConfigException>()));
      expect(() => parseTuiSection(_tui('tui:\n  classic: yes-please')),
          throwsA(isA<ConfigException>()));
      expect(() => parseTuiSection(_tui('tui:\n  statusLine: minimal')),
...
''')),
    ...
    test('CliConfig carries the fields and emits the section', () {
      ...
      final classic =
          CliConfig.fromYaml(loadYaml('tui:\n  classic: true\n') as YamlMap);
...
''');
    ...
    test('path abbreviation and work-root strip', () {
      expect(
        abbreviateSegmentPath('/Users/ag/work/x',
            homeDir: '/Users/ag',
            abbreviate: true,
            maxLength: 40,
      ...
      expect(
        abbreviateSegmentPath('/Users/ag/elsewhere',
            homeDir: '/Users/ag',
            abbreviate: true,
            maxLength: 40,
            workRoot: null,
      ...
      expect(
        abbreviateSegmentPath('/Users/ag',
            homeDir: '/Users/ag',
            abbreviate: true,
            maxLength: 40,
            workRoot: null,
      ...
      expect(
        abbreviateSegmentPath('/a/very/long/path/that/overflows',
            abbreviate: false, maxLength: 10, stripWorkPrefix: false),
        '/a/very/l…',
      );
    });
    ...
    test('clock and duration formats', () {
      expect(
        formatStatusLineClock(DateTime(2026, 9, 22, 21, 3, 7),
            clock24h: true, showSeconds: false),
        '21:03',
      );
      expect(
        formatStatusLineClock(DateTime(2026, 9, 22, 21, 3, 7),
            clock24h: true, showSeconds: true),
        '21:03:07',
      );
      expect(
        formatStatusLineClock(DateTime(2026, 9, 22, 21, 3),
            clock24h: false, showSeconds: false),
        '9:03 PM',
      );
      expect(formatStatusLineDuration(const Duration(hours: 1, minutes: 2,
          seconds: 3)), '1:02:03');
      expect(formatStatusLineDuration(
          const Duration(minutes: 21, seconds: 3)), '21:03');
    });
    ...
    test('write-time style seam resolves through the role table', () {
      ...
      expect(
        identical(
          statusLineStyle(StatusLineRoleKey.model,
              theme: theme, idle: false),
          model,
        ),
        isTrue,
      );
      ...
      expect(
        identical(
          statusLineStyle(StatusLineRoleKey.brandA,
              theme: theme, idle: true, brandT: 1.0),
          brand,
        ),
        isTrue,
      );
      final mid = statusLineStyle(StatusLineRoleKey.brandA,
          theme: theme, idle: true, brandT: 0.5);
