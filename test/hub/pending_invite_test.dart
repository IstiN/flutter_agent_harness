...
void main() {
  test('config back-compat: missing/non-array invites → [], persist '
    ...
    await File(cfgFile).writeAsString(
      jsonEncode({
        'url': 'ws://legacy:9/ws',
        'name': 'legacy',
        'channels': ['ops'],
  ...
  test(
    ...
    () async {
      ...
      final down = await PendingInvites(
        client: offline,
        configFile: cfgFile,
      ).invite('carol');
  ...
  test(
    '16-hex agent id: immediate chankey DM on the explicit channel',
    () async {
      final hub = FakeHub();
      await hub.start();
      addTearDown(() => hub.stop());
  ...
  test(
    'ambiguous name: honest error listing both ids, nothing armed',
    () async {
      final hub = FakeHub();
      await hub.start();
      addTearDown(() => hub.stop());
  ...
  test(
    'shared-config self-invite guard: own name never self-delivers',
    () async {
      final hub = FakeHub();
      await hub.start();
      addTearDown(() => hub.stop());
