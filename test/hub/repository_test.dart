...
void main() {
  ...
  test(
    'repository delivers inbound to the steering source and drains once',
    () async {
  ...
  test(
    'HubPlugin: config wiring, persisted 0600 key, steering delivery',
    () async {
  ...
  test('HubPlugin.connectTo: bare host + name + channel → new identity on '
    ...
    final result = await plugin.connectTo(
    ...
    expect(
      await loadChannelKeys('${tmp.path}/.dap/channels.json'),
      contains('lobby'),
    );
