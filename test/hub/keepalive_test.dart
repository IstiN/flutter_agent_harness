  ...
  Future<void> _upgradeThenIgnore(Socket socket) async {
    ...
    await for (final data in socket) {
      ...
      final accept = base64Encode(
        (await Sha1().hash(utf8.encode(key + _wsGuid))).bytes,
      );
      socket.add(
        utf8.encode(
...
void main() {
  ...
  test(
    'keepalive: answering peer survives ping cycles and stays usable',
    () async {
      final hub = FakeHub();
      await hub.start();
  ...
  test(
    'honest failure: sendToChannel/sendDm while disconnected throw',
    () async {
      final client = HubClient(
        config: HubConfig(
      ...
      await expectLater(
        client.sendToChannel('general', 'anyone?'),
        throwsA(isA<StateError>()),
      );
      await expectLater(
        client.sendDm('a_0123456789abcdef', 'anyone?'),
        throwsA(isA<StateError>()),
      );
    },
    timeout: timeout,
