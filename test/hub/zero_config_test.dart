...
void main() {
  test('settings precedence: env > yaml > ~/.dap/config.json > defaults; '
    ...
    final hostname = Platform.localHostname.replaceAll(
  ...
  test('auto-keygen: first send persists keys; a second instance picks them '
    ...
    final joinedRandom = hub.joins.firstWhere(
      (j) => j.agentId == a.agentId && j.channel == 'random',
    );
  ...
  test('invite: chankey E2E DM auto-persists + joins + notices on the '
    ...
    await bJoined;
    expect(
      (await loadChannelKeys(fileB))['general']!.pub,
    ...
    final inbound = b.inbound.firstWhere(
      (m) => m.plaintext == 'welcome aboard',
    );
    await a.sendToChannel('general', 'welcome aboard');
    expect(
      (await inbound.timeout(const Duration(seconds: 5))).plaintext,
      'welcome aboard',
    );
  ...
  test('HubPlugin zero-config: no yaml url/keyPath defaults still work — '
    ...
    final lobbyJoin = hub.joins.firstWhere(
      (j) => j.channel == 'lobby',
    ); // auto-join
    await plugin.start();
    ...
    // the welcome.
    expect(
      (await lobbyJoin.timeout(const Duration(seconds: 5))).agentId,
      plugin.agentId,
    );
    expect(
      plugin.repository!.client.channelStore?.pubOf('lobby'),
      lobbyKeys.pub,
    );
