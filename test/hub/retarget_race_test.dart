...
class GatedProxy {
  ...
  static Future<GatedProxy> start(InternetAddress host, int port) async {
    ...
    listener.listen((client) async {
      ...
      client.listen(
        (List<int> data) => backend.add(data),
        onError: (Object _) {},
...
void main() {
  ...
  test(
    'a cycle parked mid-connect by a retarget never hellos when released',
    () async {
      ...
      unawaited(
        client
            .retarget(
              ...
              keys: await HubIdentity.generate(),
            )
      ...
      final id = await client.retarget(
      ...
      final inbound = await client.inbound.first.timeout(
  ...
  test(
    'an undecryptable inbound DM surfaces a marker, never silence',
    () async {
      final client = HubClient(
        config: HubConfig(url: hub.url.toString()),
        identity: await HubIdentity.generate(),
