...
- [ ] `packages/fa_llm/pubspec.yaml`: remove `publish_to: none`; add
...
- [ ] `packages/fa_llm/CHANGELOG.md`: add `## 0.1.0` entry.
- [ ] `cd packages/fa_llm && dart pub publish --dry-run` — must be clean
      (no git deps inside `fa_llm` itself; its deps are `http`/`meta`, fine).
- [ ] Publish: `dart pub publish` (uses the existing pub credentials of the
      maintainer; if not logged in — `dart pub login`).
- [ ] Verify `https://pub.dev/api/packages/fa_llm` returns 0.1.0.

### 0.2 Fix + publish `flutter_agent_memory`

- [ ] Switch dep: `fa_llm: ^0.1.0` (remove the git block), remove
      `publish_to: none`, bump `version: 0.1.0`.
- [ ] Fix the `print()` in `kb_search_engine.dart` (injectable logger).
- [ ] Add the lazy tag index to `KBSearchEngine` (or a scoped, tested
      minimal version of it); keep the heuristic scoring unchanged.
- [ ] `CHANGELOG.md`: `## 0.1.0` — levels/provenance/revisions/dedup/new
      storages + the fixes above.
- [ ] `dart test --exclude-tags integration` green; `dart analyze` clean;
      `dart format --set-exit-if-changed lib test bin` clean.
- [ ] `dart pub publish --dry-run` clean; check the pana report
      (`dart pub global run pana --no-warning .` if available) — aim ≥ 130
      points, fix trivial doc issues.
- [ ] `dart pub publish`.
- [ ] Verify pub.dev page: version 0.1.0, platforms (the web export must keep
...
- [ ] Document in the memory repo's README: "`KbStorage` is the integration
      ...
      any implementation."
- [ ] Confirm the web-safe export `flutter_agent_memory_web.dart` covers
