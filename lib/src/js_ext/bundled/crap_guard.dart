/// The bundled `crap-guard` reference extension (issue #32): post-edit CRAP
/// + file-size guard wired to `crap4dart` and the repo's 2800-line cap.
///
/// Both members are const Strings compiled into the Dart binary (NOT prompt
/// files). `kCrapGuardMainJs` is a raw Dart string — the JS inside must stay
/// free of `'''` so the raw literal survives verbatim.
///
/// Behavior contract:
/// - `afterToolCall`: guards `write`/`edit` of non-generated `.dart` files.
///   Edits inside a 2s debounce window accumulate; the window close runs ONE
///   `dart pub global run crap4dart analyze` (180s timeout) plus per-file
///   line counts, and returns a single aggregated `{append}`. Findings:
///   files over 2800 lines, and per-method CRAP scores above the
///   `threshold` from `crap4dart.yaml` (fallback 12.0). A missing crap4dart
///   install produces exactly one degradation note per session (E17), then
///   the guard idles.
/// - `onSessionEnd`: when at least one guarded edit happened, runs the full
///   analyze once and reports offenders via `session.appendNote` +
///   `session.enqueueFollowUp`.
/// - Registers nothing else (no tools, slash commands, or flows).
library;

/// Exact `manifest.json` of the bundled crap-guard extension.
///
/// Declares only what the guard needs: exec for `dart` (the crap4dart
/// invocation), read-only fs (`crap4dart.yaml` + edited-file line counts),
/// and the two hooks it registers.
const String kCrapGuardManifestJson =
    '{"name":"crap-guard","kind":"cli-extension","version":"1.0.0",'
    '"description":"Post-edit CRAP + file-size guard wired to crap4dart and '
    'the repo line cap","platforms":["cli","macos","linux","windows"],'
    '"capabilities":{"exec":{"allowedCommands":["dart"]},"fs":{"read":true},'
    '"hooks":["afterToolCall","onSessionEnd"]}}';

/// `main.js` of the bundled crap-guard extension.
///
/// ES2020, no modules; touches only `jsr.ext.*`, `Date.now`, and `JSON` so it
/// runs identically on every engine adapter (qjs, flutter_js, worker, node).
/// Table-row parsing follows the real `crap4dart analyze` console format
/// (whitespace-separated columns `CRAP COV% BR% CC Class.method file:line`,
/// see crap4dart `lib/src/crap/crap_report.dart`).
const String kCrapGuardMainJs = r'''
