...
/// Prompt for folding new messages into an existing compaction checkpoint.
/// Forked from pi's UPDATE_SUMMARIZATION_PROMPT; body wording diverges
/// deliberately (no s-word framing).
///
...
/// Prompt for checkpointing the prefix of a split turn during compaction.
/// Forked from pi's TURN_PREFIX_SUMMARIZATION_PROMPT; body wording diverges
/// deliberately (no s-word framing).
///
/// Source: `prompts/compaction/turn_prefix.md`.
const turnPrefixSummarizationPrompt =
    'This is the PREFIX of a turn that was too large to keep. The SUFFIX (recent work) is retained.\n\nCheckpoint the prefix so the retained suffix stays understandable. Preserve every fact, path, error message, open request, and important tool results (test verdicts, command outputs, error traces) — the kept suffix has no other source for them. This is a lossless handoff, not a digest:\n\n## Original Request\n[What did the user ask for in this turn?]\n\n## Early Progress\n- [Key decisions and work done in the prefix]\n\n## Context for Suffix\n- [Information needed to understand the retained recent work]\n\nStay tight in wording, never in the facts the kept suffix depends on.';
...
/// SQLite subsection of the read tool description, gated on host engine
/// availability (issue 19).
///
/// Source: `prompts/tools/read_sqlite.md`.
const readSqliteSectionPrompt =
...
const lspToolDescriptionPrompt =
...
/// Description of the compact_expand tool that surfaces hidden context segments
/// and pulls a hidden or compacted one back into the conversation by its
/// numeric marker id (issue
///
...
/// Renders the agent's long-term memory entries into the system prompt so the
/// model has durable cross-session context. Entries are pulled from the project
/// and user scopes and capped at ~2 KiB.
///
/// Source: `prompts/memory/memory_section.md`.
const memorySectionPrompt = '{{memorySection}}';

/// Prompt that mines a compacted conversation span for durable long-term memory
/// entries.
///
/// Source: `prompts/memory/extract_durable.md`.
const memoryExtractDurablePrompt =
