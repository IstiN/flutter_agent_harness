...
/// One-time CSRF state for the AIIN sign-in flow (32 random bytes,
/// base64url without padding).
String aiinGenerateState() {
  final random = Random.secure();
  final bytes = List<int>.generate(32, (_) => random.nextInt(256));
  return base64Url.encode(bytes).replaceAll('=', '');
}

/// The hosted AIIN sign-in page — the recommended entry point for clients.
/// The page lists every enabled provider (Apple included once enabled),
/// runs the whole OAuth round-trip on AIIN's side (including silent
/// pass-through for an existing AIIN session) and redirects back to
/// [redirectUri] with `code` + our [state]. The state is generated by the
/// CLIENT (not the server) and validated on the caught redirect.
Uri buildAiinLoginUrl({
  ...
  return Uri.parse('$authBaseUrl/login').replace(
    queryParameters: {
