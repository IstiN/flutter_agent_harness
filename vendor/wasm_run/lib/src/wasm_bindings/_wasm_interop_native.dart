...
class _Instance extends WasmInstance {
  ...
  Future<void> runWasiStartAsync() async {
    final start = getFunction('_start');
    final init = getFunction('_initialize');
    final func = (start?.params.isEmpty ?? false)
        ? start
        : (init?.params.isEmpty ?? false)
        ? init
        : null;
    if (func == null) {
    ...
    try {
      await builder.mod.callFunctionHandle(
        func: (func as _WasmFunction).func,
        args: const [],
      );
    } on Object catch (e) {
      // wasmi reports a normal WASI proc_exit(0) as an exception. Treat it
      // as a successful command completion so stdout/stderr are preserved.
      final msg = e.toString();
      if (!msg.contains('Exited with i32 exit status 0')) rethrow;
    }
  }
