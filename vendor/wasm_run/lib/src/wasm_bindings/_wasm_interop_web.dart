...
class _Builder extends WasmInstanceBuilder {
  ...
  Future<WasmInstance> build() async {
    final instance = await Instance.fromModuleAsync(
      _module,
      importMap: _mapImports(),
    );
...
class _Instance extends WasmInstance {
    ...
    exports = UnmodifiableMapView(
      Map.fromEntries(
        instance.functions.entries
            ...
            .followedBy(
              instance.globals.entries.map(
                (e) => MapEntry(e.key, _Global(e.value, null, null)),
              ),
            )
            .followedBy(
              instance.memories.entries.map((e) {
                final c = js_util.getProperty<Object>(
                  e.value.buffer,
                  'constructor',
                );
                if (js_util.getProperty<Object>(c, 'name') ==
                    'SharedArrayBuffer') {
            ...
            .followedBy(
              instance.tables.entries.map(
                (e) => MapEntry(e.key, _Table(e.value, null, null)),
              ),
            ),
      ),
    );
  ...
  Future<WasiFile?> wasiOpenFile(
    ...
    final directories = builder.wasi!.inner.fds
        .sublist(3)
...
WasmExternal _makeWasmFunction(Function value, String? name) {
  ...
  final params =
      ...
      List.filled(js_util.getProperty(value, 'length') as int, null);
