...
/// The invite's consumer format: a bare link for desktop/mobile apps, or
/// a ready CLI command for `fa`.
enum AgentInviteFormat { link, cli }
...
class _AddAgentDialogState extends State<AddAgentDialog> {
  ...
  /// What the user copies: the bare link, or the CLI one-liner wrapping
  /// it (`fa dap import '…'` — today's CLI surface; network-scope CLI is
  /// not shipped yet, the link is shared instead).
  String? get _payload {
      ...
      content: Column(
        ...
        children: [
          Text(
            ...
            style: TextStyle(
              color:
                  _scope == AgentInviteScope.network || widget.channel.isPublic
                  ? colors.dim
                  : colors.error,
              fontSize: 13,
            ),
