...
/// sheet — the Flutter/web [RequestSecretCallback] surface. The host chat
/// screen installs this on [FaChatService.secretRequestHandler]; the service
...
Future<RequestSecretResult?> showSecretRequestSheet(
  ...
  FaChatHost.track('secret_request', {
    'result': result == null ? 'declined' : 'granted',
  });
  return result;
}
...
class _SecretRequestSheetState extends State<SecretRequestSheet> {
  ...
  void _save() {
    ...
    if (!secretNamePattern.hasMatch(name)) {
      setState(
        () => _nameError = FaChatStrings.of(context).secretRequestInvalidName,
      );
      return;
    }
          ...
          children: [
            Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    FaChatStrings.of(context).secretRequestTitle,
                    style: theme.textTheme.titleMedium,
                  ),
                ),
                IconButton(
                  icon: const Icon(Icons.close),
                  tooltip: FaChatStrings.of(context).secretRequestNotNow,
            ...
            TextField(
              ...
              decoration: InputDecoration(
                labelText: FaChatStrings.of(context).secretRequestNameLabel,
            ...
            TextField(
              ...
              decoration: InputDecoration(
                labelText: FaChatStrings.of(context).secretRequestValueLabel,
                isDense: true,
              ),
              obscureText: true,
              autocorrect: false,
            ...
            Row(
              ...
              children: [
                TextButton(
                  onPressed: () => Navigator.of(context).pop(),
                  child: Text(FaChatStrings.of(context).secretRequestNotNow),
                ),
                const SizedBox(width: 8),
                ListenableBuilder(
                  ...
                  builder: (context, _) => FilledButton.icon(
                    ...
                    label: Text(FaChatStrings.of(context).secretRequestSave),
                  ),
                ),
              ],
            ),
